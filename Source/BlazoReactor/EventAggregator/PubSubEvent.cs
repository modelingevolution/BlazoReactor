...
public class PubSubEvent<TPayload> : EventBase
{
                                               ...
                                               bool keepSubscriberReferenceAlive, Predicate<TPayload>? filter)
    {
        ...
        switch (threadOption)
        {
            ...
            case ThreadOption.UIThread:
                ...
                subscription =
                    new DispatcherEventSubscription<TPayload>(actionReference, filterReference,
        ...
        switch (threadOption)
        {
            ...
            case ThreadOption.UIThread:
                if (SynchronizationContext == null)
                    throw new InvalidOperationException("EventAggregatorNotConstructedOnUIThread");
                subscription =
                    new AsyncDispatcherEventSubscription<TPayload>(actionReference, filterReference,
                                                                   SynchronizationContext);
                break;
            default:
                subscription = new AsyncEventSubscription<TPayload>(actionReference, filterReference);
    ...
    public override Task Publish(object ev)
    {
        return InternalPublish((TPayload)ev);
    }
}
...
public abstract class PubSubEvent : EventBase
{
    ...
    public virtual void Unsubscribe(Action subscriber)
    {
        lock (Subscriptions)
        {
            IEventSubscription? eventSubscription = Subscriptions.Cast<EventSubscription>()
                                                                 .FirstOrDefault(evt => evt.Action == subscriber);
            if (eventSubscription != null)
            {
                Subscriptions.Remove(eventSubscription);
